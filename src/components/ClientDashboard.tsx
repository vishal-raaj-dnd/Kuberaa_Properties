...
import {
  ...
  MapPin, User, Phone, Mail, Shield, Award, TrendingUp,
  Building2, ArrowUpRight, AlertTriangle, Star, Menu, X
} from "lucide-react";
import { useAuth } from "../context/AuthContext";
import { getMockDashboardData } from "../data";
import { DashboardData, DocumentStep, SiteVisit, PaymentMilestone, OwnedProperty } from "../types";
...
function StatusDot({ status }: { status: "done" | "in-progress" | "pending" }) {
...
function OverviewTab({ data }: { data: DashboardData }) {
  ...
  const paidAmount = data.payments.filter(p => p.status === "paid").reduce((_, p) => p.amount, "");
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-br from-[#0B3C33] to-[#06221D] rounded-2xl border border-emerald-700/30 p-7 relative overflow-hidden">
        ...
        <h2 className="font-serif text-3xl text-[#F5EFE3] relative">{data.user.name}</h2>
        ...
        <div className="flex gap-4 mt-5 relative">
          <div className="bg-[#C8A467]/10 border border-[#C8A467]/20 rounded-xl px-4 py-2">
            ...
            <p className="font-sans text-xs text-[#F5EFE3] font-bold capitalize">{data.user.role}</p>
          </div>
          <div className="bg-[#C8A467]/10 border border-[#C8A467]/20 rounded-xl px-4 py-2">
            <p className="font-sans text-[9px] text-[#C8A467] uppercase tracking-widest">Client ID</p>
            <p className="font-sans text-xs text-[#F5EFE3] font-bold">{data.user.id.toUpperCase()}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        <StatCard label="Total Invested"   value="₹13.5L"                                sub="payments cleared" />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white/5 border border-[#F5EFE3]/8 rounded-2xl p-5 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="font-sans text-xs text-[#F5EFE3]">{data.user.email}</p>
          </div>
        </div>
        <div className="bg-white/5 border border-[#F5EFE3]/8 rounded-2xl p-5 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="font-sans text-xs text-[#F5EFE3]">{data.user.phone}</p>
          </div>
        </div>
        <div className="bg-white/5 border border-[#F5EFE3]/8 rounded-2xl p-5 flex items-center gap-3">
...
function DocumentsTab({ documents }: { documents: DocumentStep[] }) {
  return (
  ...
  const VisitCard = ({ visit }: { visit: SiteVisit }) => (
    <div className={`rounded-2xl border p-5 ${
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <span className={`text-[9px] font-bold tracking-widest uppercase px-2.5 py-1 rounded-lg border font-sans ${
      ...
      <div className="mt-4 flex items-center gap-4 text-[11px] font-sans text-[#F5EFE3]/50">
        ...
        <span>·</span>
        <span className="flex items-center gap-1.5"><User className="w-3.5 h-3.5 text-[#C8A467]" />{visit.consultant}</span>
      </div>
    </div>
  );
    ...
    <div className="space-y-8">
      {upcoming.length > 0 && (
        <div>
          ...
          <div className="space-y-3">{upcoming.map(v => <VisitCard key={v.id} visit={v} />)}</div>
        </div>
      )}
      {completed.length > 0 && (
        <div>
          <p className="font-sans text-[10px] tracking-widest uppercase text-emerald-400/70 mb-4 font-bold">Completed Visits</p>
          <div className="space-y-3">{completed.map(v => <VisitCard key={v.id} visit={v} />)}</div>
        </div>
      )}
    </div>
  );
...
function PaymentsTab({ payments }: { payments: PaymentMilestone[] }) {
...
function PropertiesTab({ properties }: { properties: OwnedProperty[] }) {
  return (
    <div className="space-y-6">
      {properties.map(prop => (
        <div key={prop.id} className="bg-white/3 border border-[#F5EFE3]/8 rounded-2xl overflow-hidden">
          ...
          <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div className="space-y-4">
              <Detail label="Plot Number"     value={prop.plotNumber} />
              <Detail label="Plot Area"       value={prop.area} />
              <Detail label="Location"        value={prop.location} />
              <Detail label="Purchase Price"  value={prop.purchasePrice} />
            </div>
            <div className="space-y-4">
              <Detail label="Purchase Date"   value={prop.purchaseDate} />
              <div>
                <p className="font-sans text-[9px] text-[#F5EFE3]/30 uppercase tracking-widest mb-1">DTCP Reference</p>
                <p className="font-mono text-xs text-[#C8A467] bg-[#C8A467]/10 px-2.5 py-1 rounded-lg inline-block">{prop.dtcpRef}</p>
              </div>
              <div>
                <p className="font-sans text-[9px] text-[#F5EFE3]/30 uppercase tracking-widest mb-1">TNRERA Reference</p>
                <p className="font-mono text-xs text-[#C8A467] bg-[#C8A467]/10 px-2.5 py-1 rounded-lg inline-block">{prop.tneraRef}</p>
              </div>
            </div>
          </div>
...
export default function ClientDashboard() {
  ...
  useEffect(() => {
    ...
    const d = getMockDashboardData(user.id);
    // For new registered users without dashboard data, create a minimal dataset
    if (d) {
      setData(d);
    } else {
      // New sign-up user — show empty dashboard with their info
      setData({
        user,
        documents: [
          { id: "d1", title: "Account Created", subtitle: "Your Kuberaa client account is now active.", date: user.joinedAt, status: "done" },
          { id: "d2", title: "Property Selection", subtitle: "Browse our available estates and express interest.", date: "", status: "pending" },
          { id: "d3", title: "Agreement of Sale", subtitle: "Pending property selection.", date: "", status: "pending" },
          { id: "d4", title: "Documentation & Verification", subtitle: "Pending agreement.", date: "", status: "pending" },
          { id: "d5", title: "Registration", subtitle: "Pending documentation.", date: "", status: "pending" },
          { id: "d6", title: "Possession & Handover", subtitle: "Pending registration.", date: "", status: "pending" },
        ],
        visits: [],
        payments: [],
        properties: []
